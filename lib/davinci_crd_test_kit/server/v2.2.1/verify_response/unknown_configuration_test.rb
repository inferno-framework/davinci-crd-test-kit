...
module DaVinciCRDTestKit
  module V221
    class UnknownConfigurationTest < Inferno::Test
      ...
      run do
        load_tagged_requests(tested_hook_name, UNKNOWN_CONFIGURATION_TAG)
        ...
        requests.each do |request|
          unless request.status == 200
            add_message(
              'error',
              "Server request returned HTTP #{request.status} for a request with an unknown configuration option; " \
              'expected HTTP 200.'
            )
            next
          end
        ...
        assert_no_error_messages(
          'Responses to requests with unknown configuration were not valid. Check messages for details.'
        )
      end
    end
  end
