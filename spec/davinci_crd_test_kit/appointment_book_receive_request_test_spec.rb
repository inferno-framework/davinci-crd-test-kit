require_relative '../../lib/davinci_crd_test_kit/client_tests/appointment_book_receive_request_test'

RSpec.describe DaVinciCRDTestKit::AppointmentBookReceiveRequestTest, :request do
  ...
  let(:appointment_book_selected_response_types) { ['instructions', 'coverage_information', 'external_reference'] }

  let(:server_endpoint) { '/custom/crd_client/cds-services/appointment-book-service' }
  let(:body_json) do
    File.read(File.join(
  ...
  it 'passes and responds 200 if request sent to the provided URL and jwt `iss` claim matches the given`iss`' do
    ...
    result = run(test, crd_jwt_iss: example_client_url, appointment_book_selected_response_types:)
  ...
  it 'returns cards and systemActions and uses client information to build request' do
    ...
    run(test, crd_jwt_iss: example_client_url, appointment_book_selected_response_types:)
    ...
    expect(system_actions.first['resource']['id']).to eq('apt1')
    expect(system_actions.last['resource']['id']).to eq('apt2')

    appointment_extension = system_actions.first['resource']['extension']
    coverage_extension = appointment_extension.first['extension'].first
  ...
  it 'waits and responds with 500 if request sent to the provided URL and jwt `iss` claim mismatches the given `iss`' do
    ...
    result = run(test, crd_jwt_iss: 'example.com', appointment_book_selected_response_types:)
  ...
  it 'returns default cards when no appointment_book_selected_response_types selected' do
    allow(test).to receive(:suite).and_return(suite)
  ...
  it 'successfully returns all supported cards when all selected_response_type options are selected' do
    ...
    expect(cards.first['summary']).to eq('Appointment Book Request Form Completion Card')
    expect(cards[1]['summary']).to eq('Appointment Book Launch SMART Application Card')
    expect(cards[2]['summary']).to eq('Appointment Book External Reference Card')
    expect(cards[3]['summary']).to eq('Appointment Book Create/Update Coverage Information Card')
    expect(cards[4]['summary']).to eq('Appointment Book Instructions Card')
