require_relative '../../../../../lib/davinci_crd_test_kit/client/v2.2.1/multiple_payers/' \
                 'client_multiple_payers_workflow_test'

RSpec.describe DaVinciCRDTestKit::V221::ClientMultiplePayersWorkflowTest, :request do
  ...
  before do
    ...
    allow_any_instance_of(DaVinciCRDTestKit::HookRequestEndpoint)
      .to receive(:multiple_payers_group?).and_return(true)
  end
  ...
  it 'lists the payer organization ids in the wait dialog when provided' do
    result = run(test, cds_jwt_iss: example_client_url,
                       complete_prefetch_service_organization_id: 'org-complete',
  ...
  it 'continues waiting after a hook request is received' do
    result = run(test, cds_jwt_iss: example_client_url)
    expect(result.result).to eq('wait')
  ...
  it 'accepts a second hook request while waiting' do
    result = run(test, cds_jwt_iss: example_client_url)
    expect(result.result).to eq('wait')
    ...
    post_hook_request(hook_instance: 'second-hook-instance')
    expect(last_response).to be_ok
  ...
  it 'omits coverage information when the request disables it' do
    run(test, cds_jwt_iss: example_client_url)
