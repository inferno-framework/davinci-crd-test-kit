require_relative '../../../../../lib/davinci_crd_test_kit/client/v2.0.1/invocation/encounter_discharge_receive_request_test' # rubocop:disable Layout/LineLength

RSpec.describe DaVinciCRDTestKit::V201::EncounterDischargeReceiveRequestTest, :request do
  ...
  let(:encounter_discharge_selected_response_types) { ['instructions', 'coverage_information', 'external_reference'] }

  let(:server_endpoint) { '/custom/crd_client/cds-services/encounter-discharge-service' }
  let(:body) do
    JSON.parse(File.read(File.join(
                           __dir__, '..', '..', '..', '..', 'fixtures', 'encounter_discharge_hook_request.json'
                         )))
  end
  ...
  it 'passes and responds 200 if request sent to the provided URL and jwt `iss` claim matches the given`iss`' do
    ...
    result = run(test, cds_jwt_iss: example_client_url, encounter_discharge_response_approach: 'mocked',
  ...
  it 'returns cards and systemActions and uses client information to build request' do
    ...
    run(test, cds_jwt_iss: example_client_url, encounter_discharge_response_approach: 'mocked',
              encounter_discharge_selected_response_types:)
  ...
  it 'waits and responds with 500 if request sent to the provided URL and jwt `iss` claim mismatches the given `iss`' do
    ...
    result = run(test, cds_jwt_iss: 'example.com', encounter_discharge_response_approach: 'mocked',
                       encounter_discharge_selected_response_types:)
  ...
  it 'returns default cards when no encounter_discharge_selected_response_types selected' do
    allow(test).to receive(:suite).and_return(suite)
    ...
    expect(cards.first['summary']).to eq('Encounter Discharge Instructions Card')
  end
  ...
  it 'successfully returns all supported cards when all selected_response_type options are selected' do
    ...
    run(test, cds_jwt_iss: example_client_url,
              encounter_discharge_response_approach: 'mocked',
    ...
    expect(cards.first['summary']).to eq('Encounter Discharge Request Form Completion Card')
    expect(cards[1]['summary']).to eq('Encounter Discharge Launch SMART Application Card')
    expect(cards[2]['summary']).to eq('Encounter Discharge External Reference Card')
    expect(cards[3]['summary']).to eq('Encounter Discharge Create/Update Coverage Information Card')
    expect(cards[4]['summary']).to eq('Encounter Discharge Instructions Card')
