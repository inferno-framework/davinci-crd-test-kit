RSpec.describe DaVinciCRDTestKit::V201::ClientCardMustSupportCoverageInformationTest do
  ...
  let(:order_sign_hook_response_covered) do
    covered_path = File.join(__dir__, '..', '..', '..', '..', 'fixtures',
                             'coverage_info_system_action_covered.json')
    { cards: [], systemActions: [JSON.parse(File.read(covered_path))] }.to_json
  end
  let(:order_sign_hook_response_conditional) do
    conditional_path = File.join(__dir__, '..', '..', '..', '..', 'fixtures',
                                 'coverage_info_system_action_conditional.json')
    { cards: [], systemActions: [JSON.parse(File.read(conditional_path))] }.to_json
  end
