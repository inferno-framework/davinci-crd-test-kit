require_relative '../../lib/davinci_crd_test_kit/client_tests/order_select_receive_request_test'

RSpec.describe DaVinciCRDTestKit::OrderSelectReceiveRequestTest, :request do
  ...
  let(:order_select_selected_response_types) { ['instructions', 'coverage_information', 'external_reference'] }

  let(:server_endpoint) { '/custom/crd_client/cds-services/order-select-service' }
  let(:body) do
  ...
  it 'passes and responds 200 if request sent to the provided URL and jwt `iss` claim matches the given`iss`' do
    ...
    result = run(test, crd_jwt_iss: example_client_url, order_select_selected_response_types:)
  ...
  it 'returns cards and systemActions and uses client information to build request' do
    ...
    run(test, crd_jwt_iss: example_client_url, order_select_selected_response_types:)
  ...
  it 'waits and responds with 500 if request sent to the provided URL and jwt `iss` claim mismatches the given `iss`' do
    ...
    result = run(test, crd_jwt_iss: 'example.com', order_select_selected_response_types:)
  ...
  it 'returns default cards when no order_select_selected_response_types selected' do
    allow(test).to receive(:suite).and_return(suite)
    ...
    expect(cards.first['summary']).to eq('Order Select Instructions Card')
  end
  ...
  it 'successfully returns all supported cards when all selected_response_type options are selected' do
    ...
    run(test, crd_jwt_iss: example_client_url, order_select_selected_response_types: order_select_selected_response_types +
    ...
    expect(cards.first['summary']).to eq('Order Select Request Form Completion Card')
    expect(cards[1]['summary']).to eq('Order Select Launch SMART Application Card')
    expect(cards[2]['summary']).to eq('Order Select External Reference Card')
    expect(cards[3]['summary']).to eq('Order Select Additional Orders As Companions/Prerequisites Card')
    expect(cards[4]['summary']).to eq('Order Select Propose Alternate Request Card')
    expect(cards[5]['summary']).to eq('Order Select Create/Update Coverage Information Card')
    expect(cards[6]['summary']).to eq('Order Select Instructions Card')
