require_relative '../../../../../lib/davinci_crd_test_kit/server/v2.2.1/verify_response/unknown_context_test'

RSpec.describe DaVinciCRDTestKit::V221::UnknownContextTest do
  ...
  def create_service_request(
    ...
    repo_create(
      ...
      request_body: unknown_context_request_body.to_json,
  ...
  def create_unknown_context_request(**args)
    create_service_request(
      **args,
      tags: [DaVinciCRDTestKit::ORDER_SIGN_TAG, DaVinciCRDTestKit::UNKNOWN_CONTEXT_TAG]
    )
  end

  def unknown_context_request_body
  ...
  it 'passes when unknown context responses include coverage-info' do
    create_unknown_context_request(body: original_response)
  ...
  it 'fails if the unknown response does not contain coverage-info' do
    create_unknown_context_request(body: filtered_response)

    result = run(runnable)

    expect(result.result).to eq('fail')
    expect(result.result_message).to include('unknown context were not valid')
    expect(
  ...
  it 'omits if no unknown context follow-up request was made for secondary hooks' do
