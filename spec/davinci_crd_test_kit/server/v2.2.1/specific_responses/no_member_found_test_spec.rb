RSpec.describe DaVinciCRDTestKit::V221::NoMemberFoundTest do
  ...
  def coverage_info_system_action(covered: 'not-covered', reason: 'no-member-found')
  ...
  it 'passes when the coverage info extension is not-covered because no member was found' do
  ...
  it 'fails when the coverage info extension reason is not no-member-found' do
    result = run(runnable, coverage_info: [coverage_info_system_action(reason: 'technical')].to_json)

    expect(result.result).to eq('fail'), result.result_message
    expect(result.result_message).to match(/Not all coverage info extensions/)

    expect(entity_result_message).to match(/Coverage reason should be `no-member-found`, but found `technical`/)
  end
end
